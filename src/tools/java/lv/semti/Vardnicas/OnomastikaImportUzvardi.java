...
public class OnomastikaImportUzvardi {
	...
	public static void main(String[] args) throws Exception {
		...
		ieeja = new BufferedReader(
				new InputStreamReader(new FileInputStream("D:\\Lingvistika\\Onomastika\\cilvard"), "windows-1257"));
		while ((vārds = ieeja.readLine()) != null) {
			...
			if (irLeksikonā(w)) {
				...
				if (w.wordforms.size() == 0) {
					...
					Lexeme jaunais = analizators.createLexeme(vārds, Integer.parseInt(w.wordforms.get(0).getValue(AttributeNames.i_EndingID)),"Onomastica - cilvēkvārdi");
		...
		izeja.flush();
		analizators.toXML("D:\\Lingvistika\\java\\Morphology\\src\\main\\resources\\Lexicon ar uzvaardiem.xml");
	}
